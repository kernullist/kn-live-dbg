...
6. Command transcript and AI evidence prompts include deterministic output summaries with stdout/stderr character counts, line counts, interesting-line counts, and first/last non-empty lines before raw output. Empty stderr is omitted from the evidence prompt and from the printed report so the model does not echo a trailing `stderr` line.
