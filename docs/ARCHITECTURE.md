...
6. This is the first slice of the broader CPU-state detection family; the same read-only IOCTL pattern is intended to back future control-register, IDT/GDT, and SSDT integrity checks. Multi-group systems (more than 64 logical processors) are only partially covered by this first slice.
