...
5. The output format uses the project's `PrintColoredText` plus `KNDBG_COLOR_*` palette. `[W+X]` non-paged allocations are highlighted in red to flag BYOVD/payload-staging candidates; large-page (`PS=1`) leaves print a `LargePage` marker. `!pool summary` and the trailing `[pool.summary]` line always print and surface the raw `TotalEntries`/`NonPagedCount`/`PagedCount`/`MatchingCount`, the buffer size that finally satisfied the kernel, the retry count, and an explicit `(no SeDebugPrivilege)` warning when elevation failed.
