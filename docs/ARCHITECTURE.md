...
2. `!module integrity [module|all] [/limit <n>] [/json <path>]` iterates the loaded kernel module list, reads each live PE header, validates MZ/PE signatures and the section table, compares the module-list size to `OptionalHeader.SizeOfImage`, and checks executable or `.text` sections for both static W+X section characteristics and effective W+X page-table permissions through `TranslateVirtual`.
3. Module results keep a compact console path for suspicious evidence and a stable `kn-live-dbg.module-integrity.v1` JSON path for baseline diffing. Header read failures and invalid PE shapes are marked suspicious but do not abort the full scan.
...
6. `module.integrity` and `driver.integrity` expose these same read-only paths to the AI capability router. Their executors accept only target/limit args, reject unsafe text, require the driver device to be open, and never emit raw debugger commands.
