...
All requests include an explicit `Size` field. Variable read/write payloads use `FIELD_OFFSET(..., Data)` as the header size. ABI version 5 adds `IOCTL_KNDBG_FLUSH_VIRTUAL` plus explicit translation metadata for paging level and page-table entry physical addresses. ABI version 7 makes MDL-backed virtual reads opt-in through `KNDBG_READ_FLAG_ALLOW_MDL_FALLBACK` so broad scanners keep failed pointer probes on the safer `MmCopyMemory` path.
...
6. The release package includes `tools\update-byovd-intel.ps1` so a deployed EXE can refresh its own local catalog. Development builds find the repo script through parent-directory probing from `x64\<Configuration>`. The release package also includes the BYOVD fixture driver so `byovd fixture load` works from the EXE directory. Release packages intentionally do not include `yara64.exe`, `yara.exe`, or `libyara.dll`; operators install YARA separately, place it on `PATH`, or pass `/yara-path`.
