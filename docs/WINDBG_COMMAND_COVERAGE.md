...
13. `procctx <pid>` stores a process DTB context. `d*` and `e*` can use `/pid <pid>` or the stored context for process-aware user VA physical read/write. Display reads are sparse, so unreadable bytes stay visible as `??` instead of aborting the whole dump.
...
29. `tools\build.ps1` stamps `KnLiveDbg.exe`, `KnLiveDbg.sys`, and `KnLiveDbgProbe.sys` with an incrementing PE version. The local baseline is `0.0.0`, so the first scripted build becomes `0.0.1`. `tools\release.ps1` packages the built PE files, runtime DLLs, symbols/certs when present, and `kn-live-dbg-version.json` into `release\KnLiveDbg-<version>-<configuration>-x64.zip`.
