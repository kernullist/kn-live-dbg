...
20. `!etw [loggers|logger <index|name>]` enumerates ETW logger contexts via `nt!EtwpDebuggerData`'s 64-slot pointer array at offset `0x10`. Each `WMI_LOGGER_CONTEXT.LoggerName` (UNICODE_STRING) and `GetCpuClock` (function pointer) is read using PDB-resolved offsets when `nt!_WMI_LOGGER_CONTEXT` is exposed and using fallback offsets `0x68`/`0x28` otherwise (with a warning). `GetCpuClock` targets outside any loaded kernel module are flagged `[SUSPICIOUS]` as the canonical InfinityHook indicator.
