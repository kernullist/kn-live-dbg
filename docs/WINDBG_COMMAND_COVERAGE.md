...
16. `!dml_proc` is a native all-process listing command that does not require a DbgEng current process/thread. It resolves PID 4, reads `_EPROCESS.ActiveProcessLinks`, and walks the active process list from live kernel memory.
...
21. `u <address|symbol> [instruction-count]` resolves the address natively, reads up to `instruction-count * 16` code bytes through the driver, disassembles them with vendored Zydis, caps explicit counts at 256 instructions, and remembers the next offset for a following bare `u`. If the driver device is not open, the command falls back to the DbgEng disassembly path.
