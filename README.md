...
This tool is built for defensive Windows security research, anti-cheat research, driver diagnostics, and controlled lab analysis. It is not designed to bypass Windows Code Integrity and does not include Code Integrity bypass functionality. The downloadable package is a test-signed lab build. Loading it requires a test system configured for test-signed drivers and the corresponding public certificate. Production use requires a driver signature accepted by the target Windows code-integrity policy; a test signature is not production approval. See Microsoft's [test-signing guidance](https://learn.microsoft.com/en-us/windows-hardware/drivers/install/test-signing-a-driver-file).
...
51. `!kmon` (bare command or `!kmon start`) arms silent TI plus kernel live callbacks and stays on a live tail of unknown kernel drops/maps/hidden processes. A driver filename is not required. Every driver lifecycle event prints with no exception path -- `driver.drop_load`, `driver.official_load` / `driver.official_unload` / `driver.device` (inbox, undecoded, and bare object names included), post-arm `driver.image_only`, `driver.short_lived`, `driver.mapped_residue` (live pool PE, unbacked `DRIVER_OBJECT`, orphan kpage PE/W+X, headerless import-stub pool code during the watch and idle, BYOVD vehicle, plus `!mapper` leftovers), `hook.unbacked` (callback/input/dispatch/SSDT/IDT/MSR/HAL/WFP/DPC/minifilter routines outside `PsLoadedModuleList`; empty module inventory is fail-closed), `process.hidden` (ActiveProcessLinks vs SPI/Toolhelp vs handle owners vs CID), `process.masquerade` / `process.hollow` (PEB ImageBase EXE replace: unmapped/private/unbacked MEM_IMAGE, mapped-path, stamp/arch, builtin W+X, COW and reloc-aware `.text` vs disk for Windows builtins, extra PE, ghosting) / `process.implant`, `inject.remote` into/from Windows builtins or drop-path callers, `hook.window` (TI `SetWindowsHook`/`UnhookWindowsHookEx` with Steam/OBS/Discord/RTSS overlay suppression), `process.impair` (TI `TerminateProcess`/`SuspendProcess`/`ResumeProcess` on watched or builtin targets or from drop-path callers; `CreateRemoteThread` is inject.remote material), and DSE-off / CR0.WP=0. Post-load hiding is covered too, by a host-side `PsLoadedModuleList` baseline diff, a kernel-context `PsLoadedModuleList` cross view, and `\Device` back-references: `driver.vanished` (a resident module left the list with no unload event; two consecutive scans must agree within a 120s lifecycle window, or a single scan when the kernel-context cross view or the loader chain names the same module), `driver.unnotified_load` (a module appeared with no load event), `driver.remap` (same name on a new image base with no lifecycle event), `driver.tampered` (loaded image head/entry hash or `DRIVER_OBJECT` identity fields changed after the load), `driver.evidence_asymmetry` (still listed, but the service key and/or the on-disk image is gone; `reason=no_service_key` / `no_image_file` / `no_service_key_no_image_file`; a service-key-only loss on a non-inbox loader prints only when `driver.inventory_divergence`, `driver.module_chain_broken`, or an already-confirmed tamper strike names the same driver stem, and the notes carry `corroboration=<source|none>`), plus a kernel-context `PsLoadedModuleList` walk (read through the driver kernel-VA path) that is cross-checked against the host `NtQuerySystemInformation` view as `driver.inventory_divergence` (`user_view_missing` = the walk sees a module the query view dropped, `kernel_view_missing` = the mirror case), a loader-link reciprocity check as `driver.module_chain_broken` (a Flink/Blink pair that does not point back at each other), and `driver.mapped_residue layer=driver_object` for a `DRIVER_OBJECT` whose loader-entry `DllBase` is outside the list, that has neither `DriverStart` nor `DriverSection`, that only the `\Device` back-reference view finds, or that only a bounded `_OBJECT_TYPE.TypeList` sweep reaches (`type_list_only=true`, `scan=driver_object_type_list`). All of it is fail-closed: when the module inventory, symbols, service key, kernel-context walk, or `\Driver` view cannot be read the verdict is withheld and `scan_failed:modinv:*`, `scan_failed:modinv:kernelview`, or `scan_failed:drvobj:typelist` prints once (a truncated chain or a capped anonymous report adds `:truncated` / `:reportcap`). The kernel-side module-list fingerprint push is still not implemented, so a resident driver that hides and unhides inside the 2-scan/120s window can be missed. Any driver load or unload arms the capped 30s mapper-watch burst. Watched names promote on the kernel process-create callback and descendants of watched pids are auto-watched (`watch_source=child_of`), unclassified TI tasks from watched pids leave a first-seen-per-task `loader.activity` trail, watched pids get an ~8s handle-table diff reporting new device handles as `driver.handle`, and `!kmon iotrace <driver> on|off|status` (ABI 17) opts into a lab-only interposition of one named driver's `IRP_MJ_DEVICE_CONTROL` printing `driver.ioctl` first-seen per pid and IOCTL code. `/name game.exe` adds overlay-style `inject.remote`; `/name *` watches every new process. `/background` (alias `/nowatch`) arms without occupying the prompt. `add`/`remove` require an already-collecting session. Esc/q detaches; `!kmon` again reattaches. Kernel `MmCopyVirtualMemory` is not on ETW-TI; the hook and pool-PE samples are the substitute. Lab fixture: `KnLiveDbgKmonTarget.exe` (`docs/KMON_TEST_TARGET.md`); driver-interaction design in `docs/DRIVER_INTERACTION_TRACKING_DESIGN.md`.
...
53. Live-capture hardening from two Berkan loader runs against Dark and Darker: `dwm.exe` is default-watched (Microsoft-signed main-image `.text` compare, graphics vtable, heap cloned-vtable, and private-exec scans with no `!kmon watch` entry); theme/MUI resource-only PEs (`.msstyles`, `*.dll.mui`) mapped as data downgrade to one `resource_only_pe` note per pid instead of phantom implants, and near-zero-entropy RX data pages drop from the orphan report; kernel-origin user APCs/thread-context writes (System-attributed) always display instead of hiding behind caller-class gating; sustained lsass remote reads from drop/unknown callers surface as `process.credscan` (256 reads/60s per caller-target pair, signed anti-cheat paths exempt); csrss/builtin WRITEVM bookkeeping no longer classifies as `inject.remote` (raw TI log keeps everything) and a 30s image-resolution cache cut the mapper-watch arming delay that had grown to 52-146s behind event floods; and detections auto-capture the region to `<log dir>\captures` -- kpage orphans, watch-gated pool PEs, private exec/WX/PE regions, cloned-vtable tables plus redirected hook pages -- deduped with a 64MB session budget, chunked to the 64KB/1MB transfer limits with partial-prefix preservation, and write-through durable so volatile evidence survives unmaps and reboots. The Berkan
gap analysis added six more capture layers: driver_image (full kernel driver image on any
load, 16MB cap -- the Berkan driver's 2.4MB at `0xfffff804d4e20000` was detected but never
dumped), orphan_wx raised to 2MB per region (the 160MB W+X arena previously sampled at
256KB), apc_routine (the user-mode entry point of a kernel-origin APC, preserving the
injected code page), hidden_eprocess (0x800 bytes of the EPROCESS when a DKOM unlink is
detected, preserving ActiveProcessLinks/Protection state), graphics_dispatch (a new scanner
that walks dxgkrnl.sys/dxgmms2.sys/nvlddmkm/amdkmdag/igdkmd64 `.data` sections for kernel-VA
...
identification (code stomping detection). TI requests ETW stack capture with EVENT_ENABLE_PROPERTY_STACK_TRACE and preserves addresses from EVENT_HEADER_EXT_TYPE_STACK_TRACE32/64 when supplied. These are historical stack observations; missing frames, unwind limits and later code changes prevent treating them as a complete current execution path.
...
Both helpers accept `-TestCertificateThumbprint <40-hex-thumbprint>` to select a code-signing certificate with an accessible private key in `CurrentUser\My`. This selects the WDK signing identity without changing trust stores. The v0.0.33 package includes `KnLiveDbg-Lab-Test.cer`, the public certificate for this build; private keys are not packaged.
...
`validate-command-audit.ps1 -Sanitize` builds the standalone numeric/JSON parser corpus with AddressSanitizer and runs the already-built executable's command corpus. The command suite now has 1,993 checks across 261 registered entries; the parser corpus has 275,002 checks. The initial [command audit](docs/COMMAND_AUDIT_20260919.md) and later [Kmon review](docs/KMON_ADVERSARIAL_REVIEW_20260920.md) preserve their own results. The Kmon core/hunting gates add 11,326 hunting checks and 54 page-coverage checks per run. Build-time validators require a source checkout and compiler; the release ZIP contains operator guides and runnable fixture tools. These checks do not execute live kernel writes, load/unload races, or commands against an external DbgEng target; use the [manual checklist](docs/MANUAL_TEST_CHECKLIST.md) for those paths.
...
knkd> dump-pe 0x13d657e0000 .\dwm-stage.dll /pid 1936
knkd> !pool pe
...
| `SetWindowsHook` | `SetWindowsHookEx` / `UnhookWindowsHookEx` (input and message hooks; kmon surfaces as `hook.window` with Steam/OBS/Discord/RTSS overlay suppression) |
| `TerminateProcess` | Cross-process termination (kmon `process.impair` on watched/builtin targets) |
| `CreateRemoteThread` | Remote thread creation (kmon inject.remote material) |
...
Cross-process events (`AllocVM`, `ProtectVM`, `WriteVM`, `ReadVM`, `MapView`, `QueueUserApc`, `SetThreadContext`, `Suspend`/`Resume`, `TerminateProcess`, `CreateRemoteThread`) carry a `TargetProcessId` payload field; `/name <victim>` matches them via the resolved target basename, so a loader injecting into `notepad.exe` is caught by `/name notepad.exe` even though the event fires from the loader's PID. The live-tail header surfaces this as `image=loader.exe -> target=notepad.exe(pid=...)`. `SetWindowsHook` events carry the hooking process only, so `/name` matching and kmon's `hook.window` gate key off the caller side.
...
- Shown: `driver.drop_load`, `driver.vanished` / `driver.unnotified_load` / `driver.remap` / `driver.tampered` / `driver.evidence_asymmetry` / `driver.inventory_divergence` / `driver.module_chain_broken` (post-load hiding: unlink with no unload event, unnotified appearance, rebase, image or `DRIVER_OBJECT` tamper, service-key or image-file asymmetry, kernel-context versus query-view divergence, and loader-list link breakage; the service-key-only asymmetry form and the single-scan `driver.vanished` confirmation report only with a corroborating signal), `driver.short_lived`, `driver.mapped_residue` (`layer=pool_pe` / `driver_object` / `orphan_page` / `orphan_wx` / `byovd` plus leftovers), `hook.inline` / `hook.breakpoint` (hot ntoskrnl/win32k entry prologue: a non-inbox head transfer or an `int3` trap; an in-module hotpatch, an inbox win32k forwarder, and a register-indirect head stay deferrals), `thread.unbacked` / `thread.hidden` / `thread.dkom` (kernel-context `_EPROCESS.ThreadListHead` view plus `ActiveThreads` accounting against the host Toolhelp view), `pool.hidden` / `mapper.stub` (stub thunks in kernel executable regions, split by whether the big-pool table reports the range: a silent range is `pool.hidden`, and the table alone cannot tell an allocation removed from the table from a mapping that never was a big-pool allocation), `hook.unbacked` (`layer=callback` / `input` / `dispatch` / `ssdt` / `idt` / `msr` / `hal` / `wfp` / `dpc` / `minifilter`), `process.hidden` (ActiveProcessLinks vs SPI/Toolhelp vs handle owners vs CID), `process.masquerade` (Windows-named image from a non-inbox path), `process.hollow` (PEB ImageBase EXE replace: `exe_unmapped` / `exe_private` / `exe_unbacked` / `exe_mapped_path` / `exe_wx` / `exe_no_mz` / `exe_arch` / `hollow` stamp-section, `exe_cow` / `exe_text` / `exe_text_page` / `exe_ep` for Windows builtins, `exe_peb_base`, extra PE `exe_orphan_private` / `exe_orphan_image`, `ghost`), `process.implant` (`drop_module` or `builtin_foreign_module`), `inject.remote` into/from Windows builtins or drop-path callers, `integrity.ci` (DSE off only), `integrity.cr`, `process.syscall_unnamed`, `gap.kernel_rw` once.
...
- Legacy `exe_text`/entry-point display rules retain their builtin-specific policy. The executable-section verifier separately compares all selected processes regardless of name; its `finding.code_modified` and permission/coverage records are described in the verification guide above. Empty `PsLoadedModuleList` is fail-closed so hook/pool scans do not treat every pointer as unbacked. The kernel-thread, inline-patch, and mapper-stub layers defer the same way: an unreadable thread list, entry prologue, thunk slot, or region body withholds the verdict instead of guessing and prints `scan_failed:thread:*`, `scan_failed:patch:*`, `scan_failed:mapperpool:table`, or `scan_failed:mapperpool:modules` once (`thread.scan` / `hook.scan` notes and the `coverage:patch:*` markers record a deferred pass, while an unreadable region body in the stub layer simply produces no verdict), released when the view returns. Kernel threads are scanned every 20s and hot entry prologues every 20s (10s during the mapper watch); every inline-patch verdict and the hidden-thread and thread-list (DKOM) thread verdicts need two consecutive passes, while a thread whose start address no loaded module owns prints on its first confirming pass, so a thread-creation or hotpatch transition cannot print an inline or hidden-thread verdict. What they cannot catch: a thread whose start address sits inside a loaded image, a hotpatch that stays inside the owning module or forwards into an inbox win32k image (that is how Windows itself hotpatches), a register-indirect entry transfer, a patch below the prologue (that is `hook.dataptr` / `hook.unbacked` territory), and a stub body that cannot be read or is shorter than the sampled window. A head transfer whose decoded destination is below the canonical kernel floor is not reported, because such a value is not kernel code: the 32-bit immediate of a push/ret head sign-extends into the user half, and a thunk slot can hold anything. The same floor rule keeps a stub continuation from being printed as a trampoline destination. The destination ownership test compares against module entries with a known image size, so a destination that only an entry whose size could not be read would own reads as unbacked, and the layer defers only when no sized range is available at all. The module-inventory baseline diff is fail-closed the same way: an unreadable inventory withholds `driver.vanished` / `driver.unnotified_load` / `driver.remap` and prints `scan_failed:modinv:*` once. Post-load hiding combines the host-side baseline diff with a kernel-context `PsLoadedModuleList` walk (15s; 1.5s while the mapper watch is armed; at most 16 divergences per scan), so the query-filtered case is covered without depending on lifecycle events, but the kernel-side module-list fingerprint push is still not implemented. A non-inbox `.sys` that legitimately deletes its own service key no longer surfaces `driver.evidence_asymmetry` (`reason=no_service_key`) on its own: that form is withheld unless `driver.inventory_divergence`, `driver.module_chain_broken`, or a tamper strike names the same stem, while image-file loss, inbox loaders, and the other reasons print as before. The `_OBJECT_TYPE.TypeList` sweep runs every 60s (5s during the mapper watch) and caps reports at 16 per pass; a truncated `\Driver` view withholds it (`scan_failed:drvobj:typelist`) and a capped pass adds `:reportcap`.
...
Lab fixture `KnLiveDbgKmonTarget.exe` (`docs/KMON_TEST_TARGET.md`) only mutates copies of itself under `%TEMP%\kn-live-dbg-kmon\` (named `notepad.exe` so builtin layers fire). It does not inject into inbox `notepad.exe` / `svchost.exe` and never resumes a replaced image. `KnLiveDbg.exe --self-test console` includes `kmon-artifact-primitives` and `kmon-hidden-driver-regression` (no driver). The COW half shares one sled-length contract with the fixture (`kmon_test_target/KmonTestTargetContract.h`), so a sled shorter than the contract is a hard failure instead of a silent skip, and the cross-process half prints one explicit skip reason when the fixture or process creation is unavailable.
