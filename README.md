...
47. Quiet kernel surfaces (Phase 2): `!hal` checks HalDispatchTable ownership; `!hive` walks registry hive GetCellRoutine ownership; `!token` inspects process privilege Present/Enabled masks (integrated into `!hunt` for non-system high-risk elevation); `!etw providers` / `!etw ti-cross` cover provider DKOM heuristics and TI reception silence; `!dpc` / `!timer` / `!workitem` enumerate deferred-execution callbacks with non-image high-risk policy and explicit incomplete coverage for modern work-item threadpools. Snapshot domains include `hal`, `hive`, `dpc-timer`, and token fingerprints under `process-security`.
