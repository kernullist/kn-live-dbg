...
26. Enumerates registered firmware table providers with `!fwtable` -- passively walking `nt!ExpFirmwareTableProviderListHead`, decoding `SYSTEM_FIRMWARE_TABLE_HANDLER`-style provider nodes, annotating ProviderSignature, handler module/symbol, and DriverObject owner/name/start/size, and flagging custom signatures, duplicate signatures, corrupt links, non-image handlers, and handler/DriverObject owner mismatches without invoking firmware handlers.
