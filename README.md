...
`!module integrity` reads loaded kernel module PE headers from live memory, validates section metadata, compares the module-list size with `SizeOfImage`, and page-walks executable/`.text` sections to flag effective W+X permissions. `!driver integrity` walks `\Driver`, decodes `_DRIVER_OBJECT` fields from PDB/DIA metadata, annotates `MajorFunction[]` handlers with module/symbol ownership, and flags dispatch pointers outside loaded modules or redirected into another non-kernel driver image. Both commands support stable JSON output for baseline diffing.
